...
class edit(alloc):
    ...
    ops.append(
        (
            "t.",
            "             ",
            'Edit a task. Specify an ID or the word "new" to create.',
        )
    )
    ops.append(("", "name=TEXT      ", "task's title"))
    ops.append(("", "desc=TEXT      ", "task's long description"))
    ops.append(
        ("", "assign=USERNAME", "username of the person that the task is assigned to")
    )
    ops.append(
        ("", "manage=USERNAME", "username of the person that the task is managed by")
    )
    ops.append(
        (
            "",
            "dip=TEXT       ",
            "default interested parties, comma separated usernames/emails/full names",
        )
    )
    ...
    ops.append(
        (
            "",
            "priority=PRI   ",
            "1, 2, 3, 4 or 5; or one of Wishlist, Minor, Normal, Important or Critical",
        )
    )
    ...
    ops.append(
        (
            "",
            "best=HOURS     ",
            "shortest estimate of how many hours of effort this task will take",
        )
    )
    ops.append(
        (
            "",
            "likely=HOURS   ",
            "most likely amount of hours of effort this task will take",
        )
    )
    ops.append(
        (
            "",
            "worst=HOURS    ",
            "longest estimate of how many hours of effort this task will take",
        )
    )
    ops.append(
        ("", "estimator=USERNAME", "the person who created the estimates on this task")
    )
    ops.append(
        ("", "targetstart=DATE", "estimated date for work to start on this task")
    )
    ops.append(
        (
            "",
            "targetcompletion=DATE",
            "estimated date for when this task should be finished",
        )
    )
    ops.append(
        (
            "",
            "project=ID|NAME",
            "task's project ID, or a fuzzy match for a project name.",
        )
    )
    ops.append(("", "type=TYPE      ", "Task, Fault, Message, Milestone or Parent"))
    ops.append(("", "dupe=ID        ", "task ID of the related dupe"))
    ops.append(
        ("", "pend=IDS       ", "task ID(s), comma separated, that block this task.")
    )
    ops.append(
        (
            "",
            "reopen=DATE    ",
            "Reopen the task on this date. To be used with --status=pending.",
        )
    )
    ops.append(
        (
            "",
            "status=STATUS  ",
            "inprogress, notstarted, info, client, manager, invalid, duplicate,\n"
    ...
    ops.append(
        (
            "i.",
            "             ",
            'Edit a time sheet item. Specify an ID or the word "new" to create.',
        )
    )
    ...
    ops.append(
        (
            "",
            "unit=NUM       ",
            "time sheet item unit of duration eg: 1=hours 2=days 3=week 4=month 5=fixed",
        )
    )
    ...
    ops.append(
        (
            "",
            "private=1|0    ",
            "privacy setting of the time sheet item's comment eg: 1=private 0=normal",
        )
    )
    ...
    ops.append(
        (
            "",
            "multiplier=NUM ",
            "time sheet item multiplier eg: 1=standard 1.5=time-and-a-half 2=double-time\n"
