...
class work(alloc):
    ...
    ops.append(
        (
            "m.",
            "multiplier=NUM ",
            "The rate multiplier for the time worked.\n"
    ...
    ops.append(
        ("", "tsid=ID          ", "Add time to a specific time sheet (optional).")
    )
...
"""
    ...
    def run(self, command_list):
        ...
        if prompt:
            ...
            o["date"] = input(
                "The date that the work was performed, or hit ENTER for %s: "
                % self.today()
            )
        ...
        if o["interactive"]:
            ...
            try:
                k_, v = self.get_list(
                    "task", {"taskID": o["task"], "taskView": "prioritised"}
                ).popitem()
            except Exception:
        ...
        if timeSheetID:
            if not o["quiet"]:
                self.print_table(
                    "timeSheet",
                    self.get_list("timeSheet", {"timeSheetID": timeSheetID}),
                    self.row_timeSheet,
                    sort="ID",
                )
                self.print_table(
                    "timeSheetItem",
                    self.get_list("timeSheetItem", {"timeSheetID": timeSheetID}),
                    self.row_timeSheetItem,
                    sort=["dateTimeSheetItem", "timeSheetItemID"],
                )
