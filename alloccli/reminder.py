...
class reminder(alloc):
    ...
    ops.append(
        (
            "r.",
            "               ",
            "Edit a reminder. Specify an ID or omit -r to create.",
        )
    )
    ops.append(
    ...
    ops.append(
        (
            "",
            "frequency=FREQ   ",
            "How often this reminder is to recur.\n"
    ...
    ops.append(
        (
            "",
            "notice=WARNING   ",
            "Advance warning for this reminder. Same format as frequency.",
        )
    )
    ...
    ops.append(
        (
            "T:",
            "to=PEOPLE      ",
            "Recipients. Can be usernames, full names and/or email.",
        )
    )
...
alloc reminder --title 'Name for the reminder.' --to alla"""
    ...
    def run(self, command_list):
        ...
        if o["frequency"] and not re.match(
            r"\d+[hdwmy]", o["frequency"], re.IGNORECASE
        ):
        ...
        if o["remove"]:
            o["recipients_remove"] = [
                x["personID"] for x in self.get_people(o["remove"]).values()
            ]
