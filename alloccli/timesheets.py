...
class timesheets(alloc):
    ...
    ops.append(
        (
            "",
            "csv=[WHEN]       ",
            'Return the results in CSV format. WHEN can be "auto",\n'
    ...
    ops.append(
        ("", "task=ID|NAME     ", "A task ID, or a fuzzy match for a task name.")
    )
    ops.append(
        (
            "s:",
            "status=STATUS  ",
            "The time sheets' status. Can accept multiple values.\n"
    ...
    ops.append(
        (
            "h.",
            "hours=NUM      ",
            "The time sheets must have this many hours recorded.\n"
    ...
    ops.append(
        (
            "d.",
            "date=YYYY-MM-DD",
            "If --items is specified, then match against the items' date.\n"
    ...
    ops.append(
        (
            "o:",
            "order=NAME     ",
            "The order the Time Sheets or Items are displayed in.\n"
    ...
    ops.append(
        (
            "f:",
            "fields=LIST    ",
            "The list of fields you would like printed.\n"
        ...
        if o["items"]:
            ...
            if timeSheets:
                ...
                if tids:
                    ...
                    if o["date"]:
                        # >=
                        ops["date"], ops["dateComparator"] = self.parse_date_comparator(
                            o["date"]
                        )
            ...
            if o["date"]:
                # <=
                ops["dateFrom"], ops["dateFromComparator"] = self.parse_date_comparator(
                    o["date"]
                )
            timeSheets = self.get_list("timeSheet", ops)
            self.print_table(
